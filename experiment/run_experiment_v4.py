#!/usr/bin/env python3
"""
Privacy Classifier Experiment v4 — Karpathy autoresearch iteration 4.

Changes from v3:
- Prompt F: Better distinction between "about a topic" vs "about a person"
  with explicit PUBLIC examples for generic legal/medical/financial questions
- Prompt G: Same + explicit German PUBLIC examples
- Slightly expanded regex to catch phone numbers (major leak source)
"""
...
# ============================================================
# REGEX v2 — added phone numbers (was a top leak source)
# ============================================================
...
# v4 Prompt F: Explicit topic-vs-person distinction
PROMPT_F = {
    "name": "F: topic-vs-person distinction",
    "system": """You are a privacy classifier. Respond with SENSITIVE or PUBLIC only.

SENSITIVE: The message is about a SPECIFIC PERSON's private situation. It contains someone's personal data, health details, financial specifics, legal situation, relationship, or confession. Third-party messages referencing a named person's private data count. Any language.

PUBLIC: The message asks ABOUT A TOPIC in general — even sensitive topics like divorce, medical conditions, salaries, or legal issues. If no specific person's private information is revealed, it is PUBLIC. Code, tutorials, and educational content are PUBLIC. Generic questions using "I" without revealing private specifics are PUBLIC.""",
    "template": '''Examples:
Message: "My credit score dropped to 580 after missing mortgage payments" -> SENSITIVE
...
Message: "I was arrested for DUI and need a lawyer" -> SENSITIVE
Message: "What are the penalties for a first-time DUI offense?" -> PUBLIC
Message: "What are the grounds for divorce in California?" -> PUBLIC
Message: "Was sind die Symptome von Typ-2-Diabetes?" -> PUBLIC
Message: "How much does a software engineer earn in NYC?" -> PUBLIC
Message: "Design a database schema for storing credit card info" -> PUBLIC
Message: "Was passiert bei einer Privatinsolvenz?" -> PUBLIC

Message: "{text}"
Classification:''',
}

# v4 Prompt G: Prompt F + more PUBLIC examples for hard categories
PROMPT_G = {
    "name": "G: topic-vs-person + hard PUBLIC examples",
    "system": """You are a privacy classifier. Respond with SENSITIVE or PUBLIC only.
...
PROMPTS = [PROMPT_D, PROMPT_F, PROMPT_G]


# ============================================================
# CLASSIFIER & RUNNER (same as v3)
# ============================================================
...
def main():
    ...
    n_p = sum(1 for _, e in TEST_CASES if e == "PUBLIC")
    print("=" * 70)
    print("  PRIVACY CLASSIFIER v4 — topic-vs-person prompts + phone regex")
    ...
    for model in MODELS:
        ...
        if not check_model(model):
            ...
            continue
        print(f"  Warming up...")
        classify_llm("warmup", model, PROMPT_D)
    ...
    # Final table
    print("\n\n" + "=" * 85)
    print("  FINAL — sorted by combined score (FNR + FPR, lower is better)")
    print("=" * 85)
    ...
    # Save results + best prompt
    with open("/tmp/privacy_experiment_v4_results.json", "w") as f:
        json.dump(all_results, f, indent=2)

    # Find best: lowest FNR that also has FPR < 20%
    ...
    if valid:
        ...
        for prompt in PROMPTS:
            if prompt["name"] in best["name"]:
                ...
                break
        print(f"\n  Best prompt saved to /tmp/best_prompt.json")

    print(f"\n  Results saved to /tmp/privacy_experiment_v4_results.json")
